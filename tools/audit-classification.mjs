...
function buildAudit(deals) {
  const checks = [
    ...
    {
      name: "hard_cheese_is_dairy_subcategory",
      description: "Твердий, плавлений і м'який сир мають бути підкатегоріями молочних продуктів, а не окремою верхньою категорією.",
    ...
    },
    {
      ...
      test: item => item.category === "Молочні"
        && /Твердий\/напівтвердий сир|Плавлений сир|М'які сири|Моцарела|Сулугуні/.test(item.subcategory)
    ...
    },
    {
      ...
      test: item => item.category === "Молочні"
        && !/паста[-\s]?філата|паста сирков/.test(text(item.name))
        && /каша|пластівц|мюслі|круп|вермішел|макарон|паста/.test(text(item.name))
    }
  ];
